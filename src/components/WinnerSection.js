  ...
  return (
    <WinnerHighlight>
      <WinnerImageBox
        ...
        onClick={() => winners.length > 0 && imgSrc && onOpenCarousel(year)}
        title={winners.length > 0 ? 'View winner photos' : undefined}
        ...
        {winners.length > 0 ? (
          winners.map((w) => (
            <WinnerPerson key={w.person}>
              ...
              <WinnerMeta>
                {w.diff === 0 ? 'Perfect!' : w.over ? `Over by ${w.diff}` : `Under by ${w.diff}`}
              </WinnerMeta>
            </WinnerPerson>
          ))
        ) : (
          <div style={{ fontSize: '.75rem', opacity: 0.7 }}>No valid guesses yet.</div>
        )}
      </WinnerNames>
    </WinnerHighlight>
  );
